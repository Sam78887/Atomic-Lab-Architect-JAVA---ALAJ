...
/* 
    ...
    if (celsiusBP < 25) System.out.println("Predicted Phase: Gas (at 25°C)");
...
*/
    ...
    // Check if it's an alcohol/phenol (Organic with OH)
    if (counts.getOrDefault("C", 0) > 0 && counts.getOrDefault("O", 0) > 0 && counts.getOrDefault("H", 0) > 0) {
        int c = counts.get("C");
        int h = counts.get("H");
        int o = counts.get("O");
    ...
    // Default sorting logic for non-alcohols
    StringBuilder sb = new StringBuilder();
...
/** 
    public static String generateFormula(Map<String, Integer> counts) {
     ...
     public static String generateSMILES(Atom central, Map<String, Integer> counts) {
       ...
       public static String generateSMILES(Atom central, Map<String, Integer> counts) {
    if (central.symbol.equals("C") && counts.containsKey("O")) {
        StringBuilder sm = new StringBuilder();
        // Create a simple Carbon chain based on C count
